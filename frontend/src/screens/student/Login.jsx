...
import { setCredentials } from '../../slices/StudentSlices/studentAuthSlice.js'
...
const submitHandler = async (e) => {
  e.preventDefault();
  try {
    const res = await axios.post('http://localhost:5000/api/student/auth', { email, password });
    if (res.status === 200) {
      const { studentToken } = res.data; // Make sure to extract studentToken from the response
      if (studentToken) {
        dispatch(setCredentials({ studentToken })); // Dispatch setCredentials with studentToken
  ...
  return (
    <section className="h-screen">
      <div className="container h-full px-6 py-24" style={{background:"linear-gradient(90deg,rgba(255,255,255,1) 0%, rgba(72,196,237,1) 67%)"}} >
        <div className="g-6 flex h-full flex-wrap items-center justify-center lg:justify-between">
          ...
          <div className='pb-20'><h1 className='pl-60'>Login</h1></div>
